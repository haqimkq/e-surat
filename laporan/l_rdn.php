...
$no = 1;
if (isset($_POST['cetak4'])) {
    /* $kec = $_POST['jenis']; */
    $tgl1 = $_POST['tgl1'];
    $tgl2 = $_POST['tgl2'];

    $sql = mysqli_query($link, "SELECT * FROM rdn r
  join masyarakat m on r.id_msy = m.id_msy 
  join pelayanan p on r.id_pelayanan = p.id_pelayanan WHERE r.tgl BETWEEN '$tgl1' AND '$tgl2' ORDER BY r.tgl ");
    $label = 'LAPORAN DATA LEGALISASI REKOMENDASI DISPENSASI NIKAH  <br> Tanggal : ' . tgl($tgl1) . ' s/d ' . tgl($tgl2);
    /* $label = 'LAPORAN DATA BARANG MASUK <br> JENIS : ' . $kec; */
} else {
    $sql = mysqli_query($link, "SELECT * FROM RDN r
  join masyarakat m on r.id_msy = m.id_msy 
  join pelayanan p on r.id_pelayanan = p.id_pelayanan ORDER BY r.tgl  ");
    $label = 'LAPORAN DATA LEGALISASI REKOMENDASI DISPENSASI NIKAH';
}
                ...
                <table border="1" cellspacing="1" cellpadding="6" width="100%">
                    <thead style="background-color: #5F9EA0">
                        <tr>
                            ...
                            <th style="text-align: center; font-size: 18px; width:15%;">Foto KTP </th>
                            <th style="text-align: center; font-size: 18px; width:15%;">Foto Kartu Keluarga</th>
                            <th style="text-align: center; font-size: 18px; width:10%;">Surat Dari KUA</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="w-10" align="center">
                                <?php echo "<a href='$data[ktp_p]' download><img src='$data[ktp_p]' width='120' height='90' style='border-radius: 5%;' /></a>"; ?>
                            </td>
                            <td class="w-10" align="center">
                                <?php echo "<a href='$data[kk_p]' download><img src='$data[kk_p]' width='120' height='90' style='border-radius: 5%;' /></a>"; ?>
                            </td>
                            <td class="w-10" align="center">
                                <?php
                                    if (!empty($data['s_kua'])) {
                                        $pdfPath = '../pelayanan/' . $data['s_kua'];
