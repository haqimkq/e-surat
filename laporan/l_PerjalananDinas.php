...
if (isset($_POST['cetak10'])) {
    ...
    $sql = mysqli_query($link, "SELECT * FROM perjalanan_dinas  pd 
    join pegawai p on pd.id_pegawai = p.id_pegawai  WHERE pd.tanggalPergi BETWEEN '$tgl1' AND '$tgl2' ORDER BY pd.tanggalPergi ");
    $label = 'LAPORAN DATA PERJALANAN DINAS PEGAWAI  <br> Tanggal : ' . tgl($tgl1) . ' s/d ' . tgl($tgl2);
    /* $label = 'LAPORAN DATA BARANG MASUK <br> JENIS : ' . $kec; */
} else {
    $sql = mysqli_query($link, "SELECT * FROM perjalanan_dinas  pd 
    join pegawai p on pd.id_pegawai = p.id_pegawai ORDER BY pd.tanggalPergi  ");
                    ...
                    <tbody>
                        <?php while ($data = mysqli_fetch_array($sql)) { ?>
                            <tr>
                                ...
                                <td class="w-5" align="center"><?= tgl($data['tanggalPergi']); ?></td>
                                <td class="w-5" align="center"><?= tgl($data['tanggalPulang']); ?></td>
                                <td class="w-15"><?= $data['keterangan']; ?></td>
                            </tr>
                        <?php } ?>
                    </tbody>
                </table>
