...
if (isset($_POST['cetak3'])) {
    ...
    $sql = mysqli_query($link, "SELECT * FROM s_keluarga sk
  join masyarakat m on sk.id_msy = m.id_msy 
  join pelayanan p on sk.id_pelayanan = p.id_pelayanan WHERE sk.tgl BETWEEN '$tgl1' AND '$tgl2' ORDER BY sk.tgl ");
    $label = 'LAPORAN DATA LEGALISASI SUSUNAN KELUARGA  <br> Tanggal : ' . tgl($tgl1) . ' s/d ' . tgl($tgl2);
    /* $label = 'LAPORAN DATA BARANG MASUK <br> JENIS : ' . $kec; */
} else {
    $sql = mysqli_query($link, "SELECT * FROM s_keluarga sk
  join masyarakat m on sk.id_msy = m.id_msy 
  join pelayanan p on sk.id_pelayanan = p.id_pelayanan ORDER BY sk.tgl  ");
    $label = 'LAPORAN DATA LEGALISASI SUSUNAN KELUARGA';
}
...
<head>
    ...
    <title>Laporan Legalisasi Susunan Keluarga </title>
</head>
                ...
                <table border="1" cellspacing="1" cellpadding="6" width="100%">
                    <thead style="background-color: #5F9EA0">
                        <tr>
                            ...
                            <th style="text-align: center; font-size: 18px; width:10%;">Foto KTP Pemohon</th>
                            <th style="text-align: center; font-size: 18px; width:10%;">Foto KK</th>
                            <th style="text-align: center; font-size: 18px; width:10%;">Foto KTP Kepala Keluarga</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="w-7" align="center">
                                <?php echo "<a href='$data[p_ktp]' download><img src='$data[p_ktp]' width='100' height='70' style='border-radius: 5%;' /></a>"; ?>
                            </td>
                            <td class="w-10" align="center">
                                <?php echo "<a href='$data[p_kk]' download><img src='$data[p_kk]' width='100' height='70' style='border-radius: 5%;' /></a>"; ?>
                            </td>
                            <td class="w-10" align="center">
                                <?php echo "<a href='$data[ktp_k]' download><img src='$data[ktp_k]' width='100' height='70' style='border-radius: 5%;' /></a>"; ?>
                            </td>
